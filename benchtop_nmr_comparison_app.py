...
def show_ibuprofen_spectrum_examples(model_a: NMRModel, model_b: NMRModel):
    ...
    with header_col:
        ...
        st.caption(
            "参考スペクトルは、装置の動作周波数によってピーク分離の見え方がどのように変化するかを視覚的に示すためのものです。これらのスペクトル画像は実測例をもとにした参考画像であり、本比較アプリ内で計算・生成されたものではありません。\n\n"
            "Example spectra are provided to visually illustrate how peak separation changes with operating frequency. "
            "These spectra are based on measured examples and were not calculated or generated by this comparison app."
        )
